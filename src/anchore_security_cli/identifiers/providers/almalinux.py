...
class AlmaLinux(ArchiveProvider):
    def __init__(self):
        super().__init__(
            name="AlmaLinux",
            url="https://osv-vulnerabilities.storage.googleapis.com/AlmaLinux/all.zip",
        )

    def _process_fetch(self, content_dir: str) -> list[ProviderRecord]:
        records = []
        for file in iglob(os.path.join(content_dir, "ALSA-*.json")):
