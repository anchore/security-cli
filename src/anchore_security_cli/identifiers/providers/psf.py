...
class PSF(ArchiveProvider):
    def __init__(self):
        super().__init__(
            name="Python Software Foundation Security Advisories",
            url="https://github.com/psf/advisory-database/archive/refs/heads/main.tar.gz",
        )

    def _process_fetch(self, content_dir: str) -> list[ProviderRecord]:
        records = []
        for file in iglob(os.path.join(content_dir, "advisory-database-main/advisories/**/PSF-*.json"), recursive=True):
