...
class RockyLinux(ArchiveProvider):
    def __init__(self):
        super().__init__(
            name="Rocky Linux",
            url="https://osv-vulnerabilities.storage.googleapis.com/Rocky%20Linux/all.zip",
        )

    def _process_fetch(self, content_dir: str) -> list[ProviderRecord]:
        records = []
        for file in iglob(os.path.join(content_dir, "R?SA-*.json")):
